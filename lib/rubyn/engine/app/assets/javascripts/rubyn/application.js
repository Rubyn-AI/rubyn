...
(function() {
  ...
  function initPrReview() {
    var btn = document.getElementById("run-pr-review-btn");
    if (!btn) return;
    ...
    btn.addEventListener("click", function() {
      ...
      var infoSection = document.getElementById("pr-review-info");
      var container = document.getElementById("pr-review-container");
      ...
      trackRequest(
        ...
        .then(function(data) {
          if (data.error) {
            container.innerHTML = '<div class="rubyn-tool-error"><p>' + escapeHtml(data.error) + '</p></div>';
            btn.disabled = false;
            btn.textContent = "Run PR Review";
            return;
          }

          infoSection.style.display = "none";
          renderPrReviewResponse(container, data, headers);
        })
        .catch(function(err) {
          container.innerHTML = '<div class="rubyn-tool-error"><p>Request failed: ' + escapeHtml(err.message) + '</p></div>';
          btn.disabled = false;
          btn.textContent = "Run PR Review";
        })
      );
  ...
  function renderPrReviewResponse(container, data, headers) {
    ...
    // Explanation
    if (explanation) {
      html += '<div class="rubyn-explanation">';
      html += '<div class="rubyn-explanation-content"><pre style="white-space:pre-wrap;font-family:inherit;">' + escapeHtml(explanation) + '</pre></div>';
      html += '</div>';
    }
    ...
    for (var i = 0; i < blocks.length; i++) {
      var code = blocks[i];
      var header = fileHeaders[i] || {};
      var filePath = header.path || null;
      var tag = header.tag || "UPDATED";

      html += '<div class="rubyn-code-block">';
      if (filePath) {
        html += '<div class="rubyn-code-header">';
        html += '<span class="rubyn-code-tag rubyn-code-tag--' + tag.toLowerCase() + '">' + tag.toUpperCase() + '</span>';
        html += '<span class="rubyn-code-path">' + escapeHtml(filePath) + '</span>';
        html += '<button class="rubyn-btn rubyn-btn--small rubyn-apply-btn" data-path="' + escapeHtml(filePath) + '" data-index="' + i + '">Apply</button>';
        html += '</div>';
      }
      html += '<pre><code class="language-ruby">' + escapeHtml(code) + '</code></pre>';
      html += '</div>';
    }

    // Credits and feedback
    if (data.credits_used || data.interaction_id) {
      html += '<div class="rubyn-tool-footer">';
      if (data.credits_used) {
        html += '<span class="rubyn-credits-badge">' + data.credits_used + ' credits used</span>';
      }
      html += '<span class="rubyn-feedback-placeholder"></span>';
      html += '</div>';
    }
    ...
    container.querySelectorAll(".rubyn-apply-btn").forEach(function(applyBtn) {
      applyBtn.addEventListener("click", function() {
        var idx = parseInt(applyBtn.getAttribute("data-index"));
        var path = applyBtn.getAttribute("data-path");
        applyRefactor(path, blocks[idx], headers, applyBtn);
      });
    });

    // Wire up feedback
    if (data.interaction_id) {
      var feedbackParent = container.querySelector(".rubyn-feedback-placeholder");
      if (feedbackParent) {
        appendFeedbackButtons(feedbackParent, data.interaction_id, headers);
      }
    }
  }
